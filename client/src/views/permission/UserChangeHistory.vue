<template>
  <div class="user-change-history">
    <el-card>
      ...
      <el-table :data="records" border stripe v-loading="loading" style="margin-top: 12px">
        ...
        <el-table-column label="申请日期" width="120" align="center">
          <template v-slot="{ row }">{{ formatDate(row.apply_date) }}</template>
        </el-table-column>
        <el-table-column label="实施日期" width="120" align="center">
          <template v-slot="{ row }">{{ formatDate(row.implement_date) }}</template>
        </el-table-column>
    ...
    <!-- 上传/编辑弹窗 -->
    <el-dialog :title="isEdit ? '编辑变更记录' : '上传变更记录'" v-model="showUpload" width="520px" :close-on-click-modal="false">
