...
func ListMonthlyChecks(c *gin.Context) {
	...
	query := database.GetDB().Model(&models.MonthlyCheckHistory{})
...
func CreateMonthlyCheck(c *gin.Context) {
	...
	// 构建按年份的上传路径: permission_monthly_checks/{year}/
	yearDir := filepath.Join(config.Cfg.Upload.PermissionMonthlyCheckPath, strconv.Itoa(year))
...
func UpdateMonthlyCheck(c *gin.Context) {
	...
	// 如果年份变化，需要移动文件到新目录
	if year != oldYear {
		oldYearDir := filepath.Join(config.Cfg.Upload.PermissionMonthlyCheckPath, strconv.Itoa(oldYear))
		newYearDir := filepath.Join(config.Cfg.Upload.PermissionMonthlyCheckPath, strconv.Itoa(year))
