...
![数据看板](./assets/readme/dashboard_preview.jpg)

*数据看板 - 实时监控资产、安全、权限等关键指标*
...
| 域账号自助管理 | 域用户自助登录、密码过期检测、密码修改，对接企业 AD 域控 |
...
- **密码过期检测** — 登录时自动检测 AD 密码过期状态，引导用户自助修改
- **IP 限流** — 域账号自助接口每 5 分钟 10 次频率限制
...
│   │   │   ├── password-vault/      #   密码本
│   │   │   ├── log/                 #   登录日志 & 操作日志
│   │   │   ├── account-portal/        #   域账号自助平台
│   │   │   │   ├── AccountPortalLogin.vue# 域账号登录页
│   │   │   │   ├── AccountDashboard.vue# 账号信息 & 修改密码
│   │   │   │   ── AccountChangePassword.vue# 密码过期修改页
...
│   │   ├── domain_account.go        #   域账号登录 / 信息查询 / 密码修改
│   │   ── ...                      #   其余业务处理器
│   ├── middleware/
│   │   ├── cors.go                  # 跨域中间件
│   │   ├── jwt.go                   # JWT 认证中间件
...
│   │   ├── ldap_password.go         #   LDAP 密码过期查询 & 修改服务
│   │   └── calendar.go              #   日程通知服务
...
<details>
<summary>域账号自助管理</summary>

- 域账号登录 — 独立登录页，域用户通过 AD 认证进入自助平台
- 账号信息展示 — 显示域账号、显示名、密码状态（正常 / 即将过期 / 已过期）
- 密码修改 — 旧密码验证 + 新密码复杂度校验 + 确认密码
- 密码过期检测 — 主平台登录时自动检测，过期则引导跳转修改密码页
- 密码策略 — 至少 14 位，包含大小写字母、数字、特殊字符中的至少 3 类
- IP 限流 — 公开接口每 5 分钟 10 次频率限制，防止暴力破解
- 渐变背景 + 毛玻璃卡片 + 浮动光斑动画，Apple 风格 UI 设计
</details>
